...
def split_dataset(dataset_name, partition_type, num_clients):
...
def get_testloader(dataset_name, partition_type, num_clients, batch_size):
    fds = split_dataset(dataset_name, partition_type, num_clients)
    apply_transforms = get_transforms(dataset_name)
    testset = fds.load_split("test").with_transform(apply_transforms)
    testloader = DataLoader(testset, batch_size=batch_size)
    return testloader

def get_dataloaders(partition_id, dataset_name, batch_size, partition_type, num_clients):
    fds = split_dataset(dataset_name, partition_type, num_clients)
